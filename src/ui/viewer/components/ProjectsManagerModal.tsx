...
export function ProjectsManagerModal({ isOpen, onClose }: Props) {
  ...
  const handleAddPath = async (project: ProjectRecord) => {
    const path = window.prompt('登记路径(绝对路径,会自动 realpath 规范化):');
    if (!path?.trim()) return;
    const res = await authFetch(`/api/projects-v2/${project.id}/paths`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleWriteAnchor = async (project: ProjectRecord) => {
    const cwd = window.prompt('在哪个目录写 .claude-mem 锚点文件?(绝对路径)');
    if (!cwd?.trim()) return;
    const res = await authFetch(`/api/projects-v2/${project.id}/anchor`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
            ...
            {!selected ? (
              ...
              <>
                ...
                <div style={{ marginTop: 12, marginBottom: 16 }}>
                  ...
                  <button style={C.btn} onClick={() => handleAddPath(selected)}>+ 路径</button>
                  <button style={C.btn} onClick={() => handleWriteAnchor(selected)}>写 .claude-mem 锚点</button>
