...
const BLOCKED_ENV_VARS = [
  ...
  'ANTHROPIC_AUTH_TOKEN',    // Same leak risk as ANTHROPIC_API_KEY; a token inherited from the
                             ...
                             // The fresh token from ~/.claude-mem-plus/.env is re-injected below
...
function serializeEnvFile(env: Record<string, string>): string {
  const lines: string[] = [
    ...
    '# This file stores keys and gateway settings for the claude-mem-plus memory agent',
...
export async function buildIsolatedEnvWithFreshOAuth(
  ...
  // 用户在 ~/.claude-mem-plus/.env 显式配置了凭证或 gateway 时，
  // 直接用它并跳过 OAuth 查找。裸 ANTHROPIC_BASE_URL 也算（gateway 可能 tokenless），
  // 否则回落到 OAuth 会静默把请求路到 api.anthropic.com。
  if (
...
export function getAuthMethodDescription(): string {
  ...
  if (hasAnthropicAuthToken()) {
    return 'Gateway auth token (from ~/.claude-mem-plus/.env)';
  }
