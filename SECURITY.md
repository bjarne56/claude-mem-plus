...
Only the latest released version of `claude-mem-plus` receives security updates. Please upgrade to the latest version before reporting a vulnerability.
...
If you discover a security vulnerability in claude-mem-plus, please report it by:
...
**Scope:** This policy covers the `claude-mem-plus` plugin and its bundled components (hooks, worker service, SQLite/Chroma sync, viewer UI, search/planning skills). Issues in upstream dependencies should be reported to those projects directly, but feel free to flag them to us as well.
...
- **PID File Protection:** Process IDs are stored in user's data directory (`~/.claude-mem-plus/`)
...
- `<claude-mem-plus-context>content</claude-mem-plus-context>` - System-level tag (prevents recursive storage)
...
Claude-mem stores data locally in `~/.claude-mem-plus/`:

- **Database:** SQLite3 at `~/.claude-mem-plus/claude-mem.db`
- **Vector Store:** Chroma at `~/.claude-mem-plus/chroma/`
- **Logs:** `~/.claude-mem-plus/logs/`
- **Settings:** `~/.claude-mem-plus/settings.json`

All claude-mem-plus state files (database, vector store, logs, settings, supervisor and PID files) are written to the local user directory and are not uploaded by claude-mem-plus itself. Claude-mem does not collect telemetry.

However, by design claude-mem-plus invokes upstream model providers and optional integrations to do its work, so observation/transcript/prompt content can leave the machine through those channels:
...
- **OAuth / keychain reads**: claude-mem-plus reads the Claude Code OAuth token from the platform-native credential store at spawn time. The token is injected into worker subprocesses but is not transmitted by claude-mem-plus.
- **GitHub releases / npm registry**: version-check and self-update flows fetch metadata from public registries.

Review your provider/Chroma configuration in `~/.claude-mem-plus/settings.json` and `~/.claude-mem-plus/.env` before sending sensitive content. Use `<private>...</private>` tags to keep specific content out of the local store.

## Permissions

Claude-mem requires:

- **File System:** Read/write to `~/.claude-mem-plus/` and `~/.claude/plugins/`
...
1. Keep claude-mem-plus updated to the latest version
