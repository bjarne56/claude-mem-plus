...
```bash
# Clone and build
git clone https://github.com/bjarne56/claude-mem-plus-plus.git
cd claude-mem-plus && bun install && bun run build
...
The hooks bridge Cursor's hook system to claude-mem-plus's worker API, allowing:
...
1. **Install**: `claude-mem-plus cursor install` generates initial context
2. **Stop hook**: Updates context in `.cursor/rules/claude-mem-plus-context.mdc` after each session
...
- **MCP Tools**: Configure claude-mem-plus's MCP server for `search`, `timeline`, `get_observations` tools
...
claude-mem-plus cursor install user

# Or install for current project only
claude-mem-plus cursor install
```
...
|-------------|--------|---------|
| `beforeSubmitPrompt` | `session-init.sh` | Initialize claude-mem-plus session |
...
- Initializes a new session in claude-mem-plus using `conversation_id` as the session ID
- Extracts project name from workspace root
- Outputs `{"continue": true}` to allow prompt submission

### Context Hook (`context-inject.sh`)
- Ensures claude-mem-plus worker is running before session
- Outputs `{"continue": true}` to allow prompt submission
...
- Maps them to claude-mem-plus's observation format
...
- Requests summary generation from claude-mem-plus
- **Updates context file** in `.cursor/rules/claude-mem-plus-context.mdc` for next session

## Configuration

The hooks read configuration from `~/.claude-mem-plus/settings.json`:
...
- [Claude-Mem Documentation](https://docs.claude-mem-plus.ai)
- [Cursor Hooks Reference](../docs/context/cursor-hooks-reference.md)
- [Claude-Mem Architecture](https://docs.claude-mem-plus.ai/architecture/overview)
