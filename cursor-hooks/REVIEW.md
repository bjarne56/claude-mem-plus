...
3. **Parity with Claude Code**: Matches claude-mem-plus's hook behavior where possible
...
**Purpose**: Initialize claude-mem-plus session when prompt is submitted
...
- ✅ Observation format matches claude-mem-plus structure
...
**How It Works**:
1. Fetches context from claude-mem-plus worker
2. Writes to `.cursor/rules/claude-mem-plus-context.mdc` with `alwaysApply: true`
...
6. Write context to `.cursor/rules/claude-mem-plus-context.mdc`
...
3. **Context Injection**:
   - ✅ Solved via auto-updated `.cursor/rules/claude-mem-plus-context.mdc`
