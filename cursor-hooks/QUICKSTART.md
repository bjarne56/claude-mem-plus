...
Connects claude-mem-plus to Cursor so that:
...
claude-mem-plus settings set CLAUDE_MEM_PROVIDER gemini
claude-mem-plus settings set CLAUDE_MEM_GEMINI_API_KEY your-api-key

# Option B: OpenRouter (free models available)
claude-mem-plus settings set CLAUDE_MEM_PROVIDER openrouter
claude-mem-plus settings set CLAUDE_MEM_OPENROUTER_API_KEY your-api-key
```
...
- claude-mem-plus provides search tools via MCP
...
- Check logs: `tail -f ~/.claude-mem-plus/logs/worker-$(date +%Y-%m-%d).log`
...
- Visit [claude-mem-plus docs](https://docs.claude-mem-plus.ai) for full feature set
