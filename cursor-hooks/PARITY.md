...
This document compares claude-mem-plus's Claude Code hooks with the Cursor hooks implementation to ensure feature parity.
...
**How it works**:
- Hook writes context to `.cursor/rules/claude-mem-plus-context.mdc`
...
### Tag Stripping
- **Claude Code**: Worker handles `<private>` and `<claude-mem-plus-context>` tags
...
**Overall**: The Cursor hooks implementation achieves **full functional parity** with claude-mem-plus's Claude Code hooks:
