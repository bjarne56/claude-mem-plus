# claude-mem-plus Docker harness

A minimal container for exercising claude-mem-plus end-to-end without polluting your
...
| `build.sh` | Runs `npm run build` then `docker build`. Tag defaults to `claude-mem-plus:basic`. |
...
```bash
# From the repo root:
docker/claude-mem-plus/build.sh
docker/claude-mem-plus/run.sh
```

`run.sh` drops you into `bash` inside the container with `claude` on `PATH` and
the plugin pre-staged at `/opt/claude-mem-plus`. Launch it with:

```bash
claude --plugin-dir /opt/claude-mem-plus
```

On exit, the SQLite DB survives at `./.docker-claude-mem-plus-data/claude-mem.db` on
the host — inspect with:

```bash
sqlite3 .docker-claude-mem-plus-data/claude-mem.db 'select count(*) from observations'
```

(DB filename `claude-mem.db` is unchanged — only the data-directory name moved
to `.claude-mem-plus`.)
...
running claude-mem-plus, not editing code.

On top of that:

- **Bun** (`/usr/local/bun`) — claude-mem-plus's worker service runtime
- **uv** (`/usr/local/bin/uv`) — provides Python for Chroma per `CLAUDE.md`
- **`plugin/`** copied to `/opt/claude-mem-plus` — the locally-built plugin tree
- **`/home/node/.claude`** and **`/home/node/.claude-mem-plus`** — pre-created mount points
...
| Var | Where | Purpose |
|-----|-------|---------|
| `TAG` | `build.sh`, `run.sh` | Override image tag (default `claude-mem-plus:basic`). |
| `HOST_MEM_DIR` | `run.sh` | Override host path for the persistent `.claude-mem-plus` volume (default `$REPO_ROOT/.docker-claude-mem-plus-data`). |
...
```bash
docker/claude-mem-plus/run.sh claude --plugin-dir /opt/claude-mem-plus --print "what did we learn yesterday?"
```

## Cleanup

```bash
rm -rf .docker-claude-mem-plus-data   # wipes the persistent DB + Chroma store
docker rmi claude-mem-plus:basic       # removes the image
```
