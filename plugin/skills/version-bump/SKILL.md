---
name: version-bump
description: Automated semantic versioning and release workflow for Claude Code plugins. Handles version increments across package.json, marketplace.json, plugin.json manifests, npm publishing (so `npx claude-mem-plus@X.Y.Z` resolves), build verification, git tagging, GitHub releases, and changelog generation.
---
...
3.  **Paths — every file that carries the version string**:
    - `package.json` — **the npm/npx-published version** (`npx claude-mem-plus@X.Y.Z` resolves from this)
...
7.  **Publish to npm** (this is what makes `npx claude-mem-plus@X.Y.Z` work):
    ```bash
    npm publish
    ```
    ...
    ```bash
    npm view claude-mem-plus@X.Y.Z version   # should print X.Y.Z
    ```
...
- [ ] **`npm publish` succeeded and `npm view claude-mem-plus@X.Y.Z version` confirms it** (so `npx claude-mem-plus@X.Y.Z` resolves)
