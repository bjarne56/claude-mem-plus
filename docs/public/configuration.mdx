...
| `CLAUDE_MEM_DATA_DIR`         | `~/.claude-mem-plus`                 | Data root — every other path (database, chroma, logs, settings.json, worker.pid) derives from this |
...
| `CLAUDE_MEM_OPENROUTER_APP_NAME`             | `claude-mem-plus`                | Optional: App name for analytics      |
...
| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `CLAUDE_MEM_DATA_DIR`         | `~/.claude-mem-plus`                 | Data directory location               |
...
`npx claude-mem-plus install` prompts for the Claude model (when the Claude provider is selected) and persists the choice to `~/.claude-mem-plus/settings.json`.

### Manual Configuration

Edit `~/.claude-mem-plus/settings.json`:
...
- **Production (installed plugin)**: `~/.claude-mem-plus/` (always, regardless of CLAUDE_PLUGIN_ROOT)
- **Development**: Can be overridden with `CLAUDE_MEM_DATA_DIR`

```
~/.claude-mem-plus/
├── claude-mem.db           # SQLite database
├── .install-version        # Version marker written by `npx claude-mem-plus install`/`repair`
...
- **HTTP API**: 10 endpoints on the worker service port (per-user, default `37700 + (uid % 100)`; see `~/.claude-mem-plus/settings.json`)
...
1. Open the viewer at the worker URL (default `http://127.0.0.1:<worker-port>`; the active port is the value of `CLAUDE_MEM_WORKER_PORT` in `~/.claude-mem-plus/settings.json`)
...
Settings are stored in `~/.claude-mem-plus/settings.json`:
...
Settings can be customized in `~/.claude-mem-plus/settings.json`.
...
Control which tools are excluded from observations. Edit `~/.claude-mem-plus/settings.json`:
```json
{
...
The Setup hook never installs anything — runtime install (Bun, uv, `bun install`) happens in `npx claude-mem-plus install` / `npx claude-mem-plus repair` outside the session lifecycle.
