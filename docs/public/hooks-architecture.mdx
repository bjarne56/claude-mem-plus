...
Claude-Mem uses lifecycle hook scripts across 5 lifecycle events. Runtime setup is handled out-of-band by `npx claude-mem-plus install` (and `npx claude-mem-plus repair`); the Setup hook only runs a sub-100ms `version-check.js` to flag stale installs. SessionStart runs 2 hooks in sequence: worker-service start, then context-hook.

### Setup Hook: Version Check

**Purpose:** Detect stale installs caused by external plugin upgrades and prompt the user to repair.

**Note:** Runtime installation (Bun, uv, `bun install` in the plugin cache) is performed by `npx claude-mem-plus install` and `npx claude-mem-plus repair` — the Setup hook itself never installs anything.
...
3. On mismatch, writes `run: npx claude-mem-plus repair` to stderr.
...
**Source:** `scripts/version-check.js`. The matching installer logic lives in `npx claude-mem-plus install` / `npx claude-mem-plus repair`, which install Bun + uv globally, run `bun install` in the plugin cache, and write the `.install-version` marker — all behind a visible clack spinner.
...
**Output format:**
```markdown
# [claude-mem-plus] recent context
...
<AccordionGroup>
  ...
  <Accordion title="Observations not captured">
    ...
    **Debugging:**
    1. Check database: `sqlite3 ~/.claude-mem-plus/claude-mem.db "SELECT * FROM observation_queue"`
...
- All heavy lifting (Bun + uv install, `bun install` inside the plugin cache) happens in `npx claude-mem-plus install` / `npx claude-mem-plus repair`, which run with a visible clack spinner outside the session lifecycle.
- On marker mismatch the hook prints a one-line `run: npx claude-mem-plus repair` hint to stderr and exits 0; the user opts into the slow path explicitly.
...
**Privacy guarantees:**
- All data stored locally in `~/.claude-mem-plus/claude-mem.db`
