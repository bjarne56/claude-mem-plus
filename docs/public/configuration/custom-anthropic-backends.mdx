...
description: "Point claude-mem-plus at bridged or self-hosted Anthropic-compatible API endpoints with ANTHROPIC_BASE_URL"
---

# Custom Anthropic-Compatible Backends

When you use the `claude` provider, claude-mem-plus talks to the Anthropic API through the Claude Agent SDK. By default, the SDK targets the official Anthropic endpoint, but it honors the standard `ANTHROPIC_BASE_URL` environment variable. That means you can route claude-mem-plus at any Anthropic-protocol-compatible backend — for example a corporate gateway, a regional bridge, or a third-party provider that exposes an Anthropic-shaped API — without changing any claude-mem-plus source code.

<Note>
This page documents how to **persist a custom base URL** so claude-mem-plus's worker uses it consistently. It does **not** add an OpenAI-compatible provider, and it does **not** auto-detect the bridge configuration from your shell — both of those are tracked separately in [issue #2196](https://github.com/bjarne56/claude-mem-plus-plus/issues/2196). For now, configuration is manual.
</Note>

## When to Use This

Use `ANTHROPIC_BASE_URL` if you need claude-mem-plus's observation worker to talk to:
...
1. **You write the credential** to `~/.claude-mem-plus/.env`.
...
4. **`ClaudeProvider.startSession()`** spawns the Claude Agent SDK with that isolated env (`src/services/worker/ClaudeProvider.ts:115`). The SDK reads `ANTHROPIC_BASE_URL` natively — claude-mem-plus does not parse or rewrite it.
...
### Step 1: Edit `~/.claude-mem-plus/.env`

The credentials file is a plain `KEY=VALUE` env file at `~/.claude-mem-plus/.env` (mode `0600`). Add or update the `ANTHROPIC_BASE_URL` line:
...
`CLAUDE_MEM_MODEL` (in `~/.claude-mem-plus/settings.json`) is passed straight through to the SDK. The model name **must be one your bridge accepts** — claude-mem-plus does not translate names.
...
- **`ANTHROPIC_BASE_URL` from your shell is not inherited.** `ANTHROPIC_API_KEY` is in the BLOCKED_ENV_VARS list (`src/shared/EnvManager.ts:10`) to prevent accidental billing on a shell-leaked key — `ANTHROPIC_BASE_URL` is not blocked, but it must still be set in `~/.claude-mem-plus/.env` for the worker to pick it up reliably across restarts. Do not rely on shell exports.
- **No auto-detection.** If you have already configured `ANTHROPIC_BASE_URL`, `ANTHROPIC_DEFAULT_HAIKU_MODEL`, etc. for Claude Code itself, claude-mem-plus will **not** read those today. Mirror the relevant values into `~/.claude-mem-plus/.env` and `~/.claude-mem-plus/settings.json`. See [issue #2196](https://github.com/bjarne56/claude-mem-plus-plus/issues/2196) for the auto-detect feature request.

## Related

- [Configuration](../configuration) — All claude-mem-plus settings
