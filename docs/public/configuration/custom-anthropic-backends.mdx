...
- **No auto-detection.** If you have already configured `ANTHROPIC_BASE_URL`, `ANTHROPIC_DEFAULT_HAIKU_MODEL`, etc. for Claude Code itself, claude-mem-plus will **not** read those today. Mirror the relevant values into `~/.claude-mem-plus/.env` and `~/.claude-mem-plus/settings.json`. See [issue #2196](https://github.com/thedotmack/claude-mem-plus/issues/2196) for the auto-detect feature request.
