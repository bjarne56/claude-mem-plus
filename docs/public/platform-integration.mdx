...
description: Complete reference for integrating claude-mem-plus worker service into VSCode extensions, IDE plugins, and CLI tools
icon: plug
---

<Note>
**Target Audience:** Developers building claude-mem-plus integrations (VSCode extensions, IDE plugins, CLI tools).
</Note>
...
// process.env.CLAUDE_MEM_WORKER_PORT, then ~/.claude-mem-plus/settings.json
...
**Tag Stripping:** Removes `<private>` and `<claude-mem-plus-context>` tags before storage.
...
```json Response
{
  ...
  "database": {
    "path": "~/.claude-mem-plus/claude-mem.db",
    "size": 1048576,
    "observations": 500,
    "sessions": 50,
    "summaries": 25
...
```json Response
{
  "projects": ["claude-mem-plus", "other-project", ...]
}
```
...
// Register tool
vscode.lm.registerTool('claude-mem-plus-search', searchTool);
```

#### Chat Participant Implementation

```typescript
const participant = vscode.chat.createChatParticipant('claude-mem-plus', async (request, context, stream, token) => {
...
export function registerSearchTool(context: vscode.ExtensionContext) {
  ...
  const searchTool = vscode.lm.registerTool('claude-mem-plus-search', {
    description: 'Search persistent memory for observations, sessions, and prompts',
    inputSchema: {
      type: 'object',
      properties: {
...
<CardGroup cols={2}>
  <Card title="Documentation" icon="book" href="https://claude-mem-plus.ai">
    Complete claude-mem-plus documentation
  </Card>
  <Card title="GitHub" icon="github" href="https://github.com/bjarne56/claude-mem-plus-plus">
