...
description: "Add persistent memory to Gemini CLI with claude-mem-plus"
---
...
### Step 1: Install claude-mem-plus
...
npx claude-mem-plus status

# Check hooks are installed — look for claude-mem-plus entries
cat ~/.gemini/settings.json | grep claude-mem-plus
```
...
On session start, you'll see claude-mem-plus context injected with your recent observations and project history.
...
1. Verify hooks exist in settings:
   ...
   ```
   You should see entries like `"SessionStart"`, `"AfterTool"`, etc. with claude-mem-plus commands.
...
# View logs
npx claude-mem-plus logs

# Restart worker
npx claude-mem-plus restart
```
...
3. Check your AI provider is configured in `~/.claude-mem-plus/settings.json`

### Raw escape codes in output

If you see characters like `[31m` or `[0m` in the session context, your claude-mem-plus version may need updating:
