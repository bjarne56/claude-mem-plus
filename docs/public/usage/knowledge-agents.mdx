...
```
build_corpus name="hooks-expertise" query="hooks architecture" project="claude-mem-plus" limit=200
```
...
curl -X POST http://localhost:37777/api/corpus \
  ...
  -d '{
    ...
    "project": "claude-mem-plus",
    "types": ["decision", "discovery"],
    "limit": 200
  }'
```

Under the hood, `CorpusBuilder` searches your observations, hydrates full records, parses structured fields (facts, concepts, files), calculates stats, and writes everything to `~/.claude-mem-plus/corpora/hooks-expertise.corpus.json`.
