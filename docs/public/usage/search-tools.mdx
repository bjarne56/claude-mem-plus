...
Instead of fetching all historical data upfront (expensive), claude-mem-plus uses a progressive disclosure approach:
...
**MCP Server:** Located at `~/.claude/plugins/marketplaces/bjarne56/plugin/scripts/mcp-server.cjs`

**Worker Service:** Express API on port 37777, managed by Bun

**Database:** SQLite FTS5 full-text search on `~/.claude-mem-plus/claude-mem.db`
