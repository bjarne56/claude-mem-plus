...
This document describes the PM2 to Bun migration that occurred in v7.1.0 (December 2025). If you're installing claude-mem-plus for the first time, this migration has already been completed and you can use the current Bun-based system documented in the main guides.
...
- **Process Name**: `claude-mem-plus-worker`
...
- **Logs**: `~/.pm2/logs/claude-mem-plus-worker-*.log`
- **PID File**: `~/.pm2/pids/claude-mem-plus-worker.pid`

**Lifecycle Commands**:
```bash
pm2 start <script>           # Start worker
pm2 stop claude-mem-plus-worker   # Stop worker
pm2 restart claude-mem-plus-worker # Restart worker
pm2 delete claude-mem-plus-worker  # Remove from PM2
pm2 logs claude-mem-plus-worker    # View logs
```
...
- **Logs**: `~/.claude-mem-plus/logs/worker-YYYY-MM-DD.log`
- **PID File**: `~/.claude-mem-plus/.worker.pid`
- **Port File**: `~/.claude-mem-plus/.worker.port` (new)
...
- Database file: `~/.claude-mem-plus/claude-mem.db` (same location)
...
if (!existsSync(pm2MigratedMarker)) {
  try {
    spawnSync('pm2', ['delete', 'claude-mem-plus-worker'], { stdio: 'ignore' });
...
`ensureWorkerRunning()` checks if `~/.claude-mem-plus/.worker.pid` exists (it doesn't for first run after update)
</Step>
<Step title="Start Worker Decision">
Worker not running → Call `startWorker()`
</Step>
<Step title="Migration Check">
Check if `~/.claude-mem-plus/.pm2-migrated` exists
</Step>
<Step title="PM2 Cleanup">
Execute `pm2 delete claude-mem-plus-worker` (errors ignored), create marker file
</Step>
...
**Location**: `~/.claude-mem-plus/.pm2-migrated`
...
4. **PM2 cleanup**: `pm2 delete claude-mem-plus-worker` (old worker terminated)
5. **Marker creation**: `~/.claude-mem-plus/.pm2-migrated` with timestamp
...
| `pm2 stop claude-mem-plus-worker` | `npm run worker:stop` | Stop worker |
| `pm2 restart claude-mem-plus-worker` | `npm run worker:restart` | Restart worker |
| `pm2 delete claude-mem-plus-worker` | `npm run worker:stop` | Remove worker |
| `pm2 logs claude-mem-plus-worker` | `npm run worker:logs` | View logs |
| `pm2 describe claude-mem-plus-worker` | `npm run worker:status` | Detailed status |
...
**Logs**:
```
Old: ~/.pm2/logs/claude-mem-plus-worker-out.log
     ~/.pm2/logs/claude-mem-plus-worker-error.log

New: ~/.claude-mem-plus/logs/worker-YYYY-MM-DD.log
```

**PID Files**:
```
Old: ~/.pm2/pids/claude-mem-plus-worker.pid

New: ~/.claude-mem-plus/.worker.pid
```
...
New: ~/.claude-mem-plus/.worker.pid
     ~/.claude-mem-plus/.worker.port
     ~/.claude-mem-plus/.pm2-migrated (all platforms)
```

**Database** (unchanged):
```
Same: ~/.claude-mem-plus/claude-mem.db
```
...
├────┼────────────────────┼─────────┼─────────┼──────────┤
│ 0  │ claude-mem-plus-worker  │ online  │ 0       │ 2d 5h    │
└────┴────────────────────┴─────────┴─────────┴──────────┘
```
...
# Or just remove claude-mem-plus logs
rm -f ~/.pm2/logs/claude-mem-plus-worker-*.log
rm -f ~/.pm2/pids/claude-mem-plus-worker.pid
```
...
~/.pm2/
├── logs/
│   ├── claude-mem-plus-worker-out.log
│   └── claude-mem-plus-worker-error.log
├── pids/
│   └── claude-mem-plus-worker.pid
└── pm2.log
```
...
**Symptoms**:
- `pm2 list` still shows `claude-mem-plus-worker`
...
```bash
# Manual cleanup
pm2 delete claude-mem-plus-worker
pm2 save  # Persist the deletion

# Force re-migration (optional)
rm ~/.claude-mem-plus/.pm2-migrated
...
rm ~/.claude-mem-plus/.worker.port
...
# 2. Start PM2 worker
pm2 start plugin/scripts/worker-cli.js --name claude-mem-plus-worker
...
# 5. Verify migration
pm2 list  # Should NOT show claude-mem-plus-worker
cat ~/.claude-mem-plus/.pm2-migrated  # Should exist
