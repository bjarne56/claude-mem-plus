...
(Runtime setup is handled out-of-band by `npx claude-mem-plus install` / `npx claude-mem-plus repair`. The Setup phase runs a sub-100ms `version-check.js` that prompts the user to repair if the `.install-version` marker is stale.)
...
// Removes <private>...</private> and <claude-mem-plus-context>...</claude-mem-plus-context>
...
<claude-mem-plus-context>...</claude-mem-plus-context>
```
