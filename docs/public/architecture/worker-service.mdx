...
- **Port**: Per-user default `37700 + (uid % 100)` (override with `CLAUDE_MEM_WORKER_PORT`). The active port is stored in `~/.claude-mem-plus/settings.json` and reported by `GET /api/health`.
...
- PID file tracking at `~/.claude-mem-plus/worker.pid`
...
Bun is required to run the worker service. If Bun is not installed, `npx claude-mem-plus install` (and `npx claude-mem-plus repair`) installs it globally during setup, with a visible clack spinner:
...
Set the Claude model used for compression via environment variable or `~/.claude-mem-plus/settings.json`:
...
- **Override**: Set `CLAUDE_MEM_WORKER_PORT` (env or `~/.claude-mem-plus/settings.json`)
- **Discovery**: `GET /api/health` returns the active port; `~/.claude-mem-plus/settings.json` stores the configured value
