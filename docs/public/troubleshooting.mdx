...
1. Find the worker port. The default is `37700 + (uid % 100)`. The configured port is the value of `CLAUDE_MEM_WORKER_PORT` in `~/.claude-mem-plus/settings.json`; the running worker also reports it on `/api/health`:
   ```bash
   PORT=$(jq -r .CLAUDE_MEM_WORKER_PORT ~/.claude-mem-plus/settings.json)
   lsof -i :$PORT
...
1. Check browser localStorage is enabled:
   ```javascript
   // In browser console
   localStorage.getItem('claude-mem-plus-settings')
   ```
...
3. Clear localStorage and try again:
   ```javascript
   // In browser console
   localStorage.removeItem('claude-mem-plus-settings')
   ```
...
1. Clear install cache:
   ```bash
   rm ~/.claude/plugins/marketplaces/bjarne56/.install-version
   ```
...
3. Check version marker:
   ```bash
   cat ~/.claude/plugins/marketplaces/bjarne56/.install-version
   cat ~/.claude/plugins/marketplaces/bjarne56/package.json | grep version
   ```
...
3. Check database for corruption:
   ```bash
   sqlite3 ~/.claude-mem-plus/claude-mem.db "PRAGMA integrity_check;"
   ```
...
3. Verify database has observations:
   ```bash
   sqlite3 ~/.claude-mem-plus/claude-mem.db "SELECT COUNT(*) FROM observations;"
   ```
...
1. Check if summaries exist:
   ```bash
   sqlite3 ~/.claude-mem-plus/claude-mem.db "SELECT COUNT(*) FROM session_summaries;"
   ```
...
3. Check database size (large database = slow queries):
   ```bash
   ls -lh ~/.claude-mem-plus/claude-mem.db
   ```
...
3. Check package.json exists:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/bjarne56/package.json
   ```
...
2. Check for stale locks:
   ```bash
   lsof ~/.claude-mem-plus/claude-mem.db
   ```
...
2. Backup database:
   ```bash
   cp ~/.claude-mem-plus/claude-mem.db ~/.claude-mem-plus/claude-mem.db.backup
   ```
...
4. Nuclear option - recreate database:
   ```bash
   rm ~/.claude-mem-plus/claude-mem.db
...
1. Check FTS5 tables exist:
   ```bash
   sqlite3 ~/.claude-mem-plus/claude-mem.db "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_fts';"
   ```
...
3. Check triggers exist:
   ```bash
   sqlite3 ~/.claude-mem-plus/claude-mem.db "SELECT name FROM sqlite_master WHERE type='trigger';"
   ```
...
2. Verify FTS5 tables populated:
   ```bash
   sqlite3 ~/.claude-mem-plus/claude-mem.db "SELECT COUNT(*) FROM observations_fts;"
   ```
...
**Symptoms**: `/plugin install claude-mem-plus` fails.
...
3. Verify installation:
   ```bash
   ls -la ~/.claude/plugins/marketplaces/bjarne56/
   ```
...
jq -r .CLAUDE_MEM_WORKER_PORT ~/.claude-mem-plus/settings.json
...
**Solution**: Check `~/.claude-mem-plus/` exists and is writable.
...
2. **Create issue**: [GitHub Issues](https://github.com/bjarne56/claude-mem-plus-plus/issues)
