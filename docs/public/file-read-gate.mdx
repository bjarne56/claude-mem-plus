...
description: "How claude-mem-plus intercepts file reads to save tokens using observation history"
---
...
Projects matching patterns in `CLAUDE_MEM_EXCLUDED_PROJECTS` skip the gate entirely. Configure this in `~/.claude-mem-plus/settings.json`:
...
2. Find the claude-mem-plus hooks section under `hooks.PreToolUse` and remove the entry with the `Read` matcher.
...
The File Read Gate is one piece of claude-mem-plus's layered context strategy:
