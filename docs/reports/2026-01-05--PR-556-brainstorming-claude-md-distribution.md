Brainstorming Report: CLAUDE.md Distribution Architecture
  ...
  Potential Claude Code feature request - support command substitution in context config:
  {
    "context": {
      "sources": [
        { "command": "claude-mem-plus live-context ${CWD}" }
      ]
    }
  }
  Or folder-level .claude-context files containing just:
  exec: claude-mem-plus live-context .
  ...
  - Timeline data in separate file: claude-mem-plus-timeline.csv
  - Generated fresh on SessionStart
  - Block Claude from reading timeline file via .claude/settings.local.json: "ignorePaths": ["claude-mem-plus-timeline.csv"]
  ...
  - Writes claude-mem-plus-timeline.csv and includes in push
  ...
  - Post-pull hook: claude-mem-plus sync --from-timeline
  ...
  - claude-mem-plus-timeline.csv = Machine-generated context sync (blocked from reading, already injected)
