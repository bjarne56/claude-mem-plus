# claude-mem-plus Production Guide
...
sqlite3 ~/.claude-mem-plus/claude-mem.db "
...
If running claude-mem-plus on multiple machines, use `claude-mem-plus-sync` to keep observations in sync:

```bash
claude-mem-plus-sync push <remote-host>    # local -> remote
claude-mem-plus-sync pull <remote-host>    # remote -> local
claude-mem-plus-sync sync <remote-host>    # bidirectional
claude-mem-plus-sync status <remote-host>  # compare counts
```
...
```bash
# Count errors by day
grep '\[ERROR\]' ~/.claude-mem-plus/logs/claude-mem-plus-*.log | \
  sed 's/\[20[0-9][0-9]-[0-9][0-9]-/\n&/g' | \
  grep -oP '^\[20\d{2}-\d{2}-\d{2}' | sort | uniq -c

# Find circuit-breaker trips
grep 'circuit\|Circuit\|ABANDONED\|abandoned' ~/.claude-mem-plus/logs/claude-mem-plus-*.log

# Check pending message health
grep 'CLAIMED\|CONFIRMED\|FAILED\|ABANDONED' ~/.claude-mem-plus/logs/claude-mem-plus-$(date +%Y-%m-%d).log | tail -20
```
