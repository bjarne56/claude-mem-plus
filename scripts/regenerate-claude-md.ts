...
const DB_PATH = path.join(os.homedir(), '.claude-mem-plus', 'claude-mem.db');
const SETTINGS_PATH = path.join(os.homedir(), '.claude-mem-plus', 'settings.json');
...
function walkDirectoriesWithIgnore(dir: string, folders: Set<string>, depth: number = 0): void {
  ...
  const ignorePatterns = [
    ...
    '.claude-mem-plus', '.open-next', '.turbo'
  ];
...
function cleanupAutoGeneratedFiles(workingDir: string, dryRun: boolean): void {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          try {
            ...
            if (content.includes('<claude-mem-plus-context>')) {
  ...
  for (const file of filesToProcess) {
    ...
    try {
      ...
      const stripped = content.replace(/<claude-mem-plus-context>[\s\S]*?<\/claude-mem-plus-context>/g, '').trim();
