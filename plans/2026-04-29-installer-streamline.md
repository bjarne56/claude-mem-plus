...
| Plugin cache dir helper | `src/npx-cli/utils/paths.ts:32–34` | `pluginCacheDirectory(version)` → `~/.claude/plugins/cache/bjarne56/claude-mem-plus/{version}/` |
...
case 'claude-code': {
  try {
    execSync(
      'claude plugin marketplace add bjarne56/claude-mem-plus && claude plugin install claude-mem-plus',
      { stdio: 'inherit' },
    );
...
1. On a clean machine (or after `rm -rf ~/.claude/plugins/marketplaces/bjarne56 ~/.claude/plugins/cache/bjarne56 ~/.claude-mem-plus`):
   ```bash
...
3. Simulate a stale install:
   ```bash
   rm ~/.claude/plugins/cache/bjarne56/claude-mem-plus/<version>/.install-version
   ```
